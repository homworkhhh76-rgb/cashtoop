...
let state={superAdmin:null,companies:{},keyIndex:{},retiredKeys:{},maintenance:{enabled:false},updatedAt:0};
...
function companyAccess(company){const tenantId=String(company.tenantId||company.companyId);return {tenantId,companyId:tenantId,companyKey:company.key,companyName:company.companyName,status:company.status,plan:company.plan,startAt:company.startAt,endAt:company.endAt,durationUnit:company.durationUnit,durationQuantity:company.durationQuantity,backupImportEnabled:company.backupImportEnabled===true,maintenanceMode:state.maintenance?.enabled===true,maintenanceMessage:state.maintenance?.message||'النظام قيد الصيانة حالياً',authVersion:company.authVersion,updatedAt:Date.now(),manager:{id:`ADMIN_${tenantId}`,username:company.managerUsername,password:company.managerPassword,displayName:'مدير الشركة',role:'admin',active:company.status==='active',permissions:{},authVersion:company.authVersion}}}
...
async function toggleMaintenance(){const enabled=!(state.maintenance?.enabled===true);if(enabled&&!confirm('تشغيل وضع الصيانة الآن؟ سيتم إخراج جميع مستخدمي الشركات إلى صفحة الصيانة خلال ثوانٍ.'))return;state.maintenance={enabled,startedAt:enabled?new Date().toISOString():'',updatedAt:new Date().toISOString(),message:'نعمل حالياً على تحسين النظام. يرجى الانتظار، سيعود النظام للعمل تلقائياً عند انتهاء الصيانة.'};await saveState();const companies=Object.values(state.companies||{}).filter(c=>!c.deleted);let done=0;for(const c of companies){c.authVersion=Date.now();c.updatedAt=new Date().toISOString();try{await writeCompany(c)}catch(err){console.warn('maintenance company update',c.companyId,err)}done++}renderMaintenance();toast(enabled?'تم تشغيل وضع الصيانة وإرسال الأمر لجميع الشركات.':'تم إيقاف وضع الصيانة ويمكن للمستخدمين الدخول الآن.',enabled?'warning':'success')}
function renderMaintenance(){const on=state.maintenance?.enabled===true;$('maintenanceText').textContent=on?'إيقاف وضع الصيانة':'تشغيل وضع الصيانة';$('maintenanceBtn').className=`btn ${on?'maintenance-on':'maintenance-off'}`;$('maintenanceStatus').textContent=on?'الحالة: وضع الصيانة نشط الآن، جميع الشركات موقوفة مؤقتاً.':'الحالة: البرنامج يعمل بشكل طبيعي.'}
