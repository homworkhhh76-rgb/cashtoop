...
function normalizeState(value){const s=value&&typeof value==='object'?value:{};const companies=s.companies&&typeof s.companies==='object'?s.companies:{};const keyIndex=s.keyIndex&&typeof s.keyIndex==='object'?s.keyIndex:{};const retiredKeys=s.retiredKeys&&typeof s.retiredKeys==='object'?s.retiredKeys:{};Object.values(companies).forEach(c=>{c.backupImportEnabled=c.backupImportEnabled===true;c.plan=['plus','pro','custom'].includes(String(c.plan||'').toLowerCase())?String(c.plan).toLowerCase():'pro';c.customLimits=normalizeCustomLimits(c.customLimits);c.tenantId=String(c.tenantId||c.companyId||`TENANT_${Date.now()}_${Math.random().toString(36).slice(2,8)}`);c.companyId=c.tenantId;if(c.key){const seg=safeSeg(normalizeKey(c.key));if(c.deleted===true||c.status==='deleted')retiredKeys[seg]={tenantId:c.tenantId,companyId:c.tenantId,key:normalizeKey(c.key),deletedAt:c.updatedAt||Date.now()};else keyIndex[seg]={...(keyIndex[seg]||{}),tenantId:c.tenantId,companyId:c.tenantId,key:normalizeKey(c.key)}}});return {superAdmin:s.superAdmin||null,companies,keyIndex,retiredKeys,updatedAt:Number(s.updatedAt||0)}}
...
function resetForm(){editingKey='';$('editingKey').value='';$('formTitle').textContent='إنشاء شركة ومفتاح جديد';$('companyForm').reset();$('companyKey').value=generateKey();$('durationUnit').value='month';$('durationQuantity').value=1;$('plan').value='plus';$('status').value='active';$('backupImportEnabled').value='false';fillCustomLimits({});$('cancelEdit').classList.add('hidden');planNote();updateExpiryPreview()}
