...
const fallbackBases=(Array.isArray(cfg.fallbackDatabaseURLs)?cfg.fallbackDatabaseURLs:[]).map(v=>String(v||'').replace(/\/+$/,'')).filter(Boolean);
const isMongoProxy=window.CASHTOP_FIREBASE?.backendMode==='mongodb-rtdb-api'||/\/api\/rtdb(?:$|\?)/i.test(base);
...
function requestCandidates(url){const primary=transportUrl(url),list=[primary];if(isMongoProxy){try{const parsed=new URL(primary,location.href),path=parsed.searchParams.get('path');if(path!==null){fallbackBases.forEach(fb=>{try{list.push(`${new URL(fb,location.href).href.replace(/\/+$/,'')}?path=${encodeURIComponent(path)}`)}catch(_){}})}}catch(_){}}return [...new Set(list)]}
async function request(url,options={},timeout=18000){let lastError=null;for(const target of requestCandidates(url)){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeout);try{let sendOptions={...options,signal:controller.signal,cache:'no-store'};if(isMongoProxy&&sendOptions.headers){const h=new Headers(sendOptions.headers);['cache-control','pragma','x-firebase-etag','if-match'].forEach(n=>h.delete(n));sendOptions={...sendOptions,headers:h}}if(isMongoProxy&&String(sendOptions.method||'GET').toUpperCase()==='PATCH'){const currentRes=await fetch(target,{signal:controller.signal,cache:'no-store',headers:{Accept:'application/json'}});const current=currentRes.ok?await currentRes.json().catch(()=>({})):{};const patch=parse(sendOptions.body,{});sendOptions={...sendOptions,method:'PUT',body:JSON.stringify({...((current&&typeof current==='object')?current:{}),...patch})}}let response=await fetch(target,sendOptions);if(isMongoProxy&&response.ok){const type=String(response.headers.get('content-type')||'').toLowerCase();if(type&&!type.includes('json'))throw new Error('نقطة المزامنة أعادت صفحة HTML بدلاً من JSON.')}if(!isMongoProxy&&!response.ok&&(response.status===401||response.status===403)){const token=await getAdminDatabaseToken().catch(()=>'');if(token)response=await fetch(authUrl(target,token),sendOptions)}if(!response.ok){const p=await response.json().catch(()=>null);throw new Error(String(p?.error?.message||p?.error||`Database API ${response.status}`))}return response}catch(error){lastError=error}finally{clearTimeout(timer)}}throw lastError||new Error('تعذر الاتصال بقاعدة البيانات.')}
